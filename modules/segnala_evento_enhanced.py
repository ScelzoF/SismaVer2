                    ...
                    try:
                        ...
                        try:
                            ...
                            if hasattr(response, 'data') and response.data:
                                ...
                                # Mostra dataframe
                                st.dataframe(
                                    df_segnalazioni[display_columns].rename(columns=column_names), use_container_width=True,
                                    hide_index=True
                                )
                            else:
    ...
    try:
        if os.path.exists("data/segnalazioni.json"):
            ...
            if not available_columns:
                ...
                st.dataframe(
                    df_segnalazioni[available_columns].rename(columns={col: column_names.get(col, col) for col in available_columns}), use_container_width=True,
                    hide_index=True
                )
        else:
